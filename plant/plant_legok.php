...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['create'])) {
        ...
        // Memeriksa apakah ID barang sudah ada
        $id_barang_exists = false;
        for ($i = 0; $i < $jumlah_barang; $i++) {
            $id_barang = $id_barang_prefix . '-' . ($i + 1);
            $stmt = $pdo->prepare("SELECT COUNT(*) FROM barang WHERE id_barang = ?");
            $stmt->execute([$id_barang]);
            if ($stmt->fetchColumn() > 0) {
                $id_barang_exists = true;
                break;
            }
        }

        if ($id_barang_exists) {
            $_SESSION['message'] = "Error: ID barang sudah ada!";
            $_SESSION['msg_type'] = "delete"; // Pesan kesalahan
        } else {
            // Jika ID barang tidak ada, lanjutkan untuk menyimpan barang
            for ($i = 0; $i < $jumlah_barang; $i++) {
...
<body>
    <div class="container">
        ...
        <p class="animated-text">Selamat datang di halaman detail plant Legok.</p>
